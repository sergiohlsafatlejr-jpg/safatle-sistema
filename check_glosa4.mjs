...
async function main() {
  ...
  for (const row of rows) {
    // dadosExtras já pode ser um objeto se o driver fez parse automático
  ...
  // Contar itens com glosa usando valorGlosado
  ...
  // Verificar campos alternativos valor_glosa
  ...
  // Verificar se existem itens glosados pelo Demonstrativo
  const [demoGlosa] = await conn.execute(`
    SELECT COUNT(*) as total FROM procedimentos p 
    JOIN arquivos a ON p.arquivoId = a.id 
    WHERE a.convenioId = 1 AND a.direcao = 'retornado' AND a.status = 'processado'
    AND (
      (JSON_EXTRACT(p.dadosExtras, '$.valorGlosado') IS NOT NULL AND CAST(COALESCE(JSON_EXTRACT(p.dadosExtras, '$.valorGlosado'), '0') AS DECIMAL(10,2)) > 0)
      OR (JSON_EXTRACT(p.dadosExtras, '$.valor_glosa') IS NOT NULL AND CAST(COALESCE(JSON_EXTRACT(p.dadosExtras, '$.valor_glosa'), '0') AS DECIMAL(10,2)) > 0)
    )
  `);
  console.log(`Total de itens glosados (valorGlosado OU valor_glosa > 0): ${demoGlosa[0].total}`);
