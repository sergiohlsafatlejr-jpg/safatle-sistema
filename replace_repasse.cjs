...
const start = code.indexOf('export async function getRepasseData(');
const remaining = code.substring(start);
const endMatch = remaining.match(/^}\s*$/m);
const end = start + endMatch.index + 1;

const newFunction = `export async function getRepasseData(filters: {
  userId: number;
  convenioId?: number;
...
console.log('Replaced getRepasseData with clean demonstrativo-only version');
