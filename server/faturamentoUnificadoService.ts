...
export async function reverterGlosa(params: {
  ...
  // Se foi vinculado manualmente, limpar completamente para permitir re-vinculação
  const query = `
    UPDATE conciliados_automatico 
  ...
  // Limpar registros auxiliares (manual_auxiliar) que foram criados pela vinculação multi-sobra
  if (atualizados > 0) {
    await db.execute(sql.raw(`
 ...
 * (ex: convênio troca código e devolve em linhas separadas).
 */
export async function vincularItemManual(params: {
  ...
  // 1b. Se o item já tinha um recebimento vinculado (re-vinculação), limpar auxiliares antigos
  ...
  // 4. Atualizar conciliados_automatico - vincular ao primeiro recebimentoId (referência principal)
  await db.execute(sql.raw(`
    UPDATE conciliados_automatico
  ...
  // 4b. Marcar os recebimentos extras (2º em diante) como vinculados também,
  ...
  if (params.criarRegraDePara && conc.convenioId && conc.codigoItem) {
    ...
    for (const codConv of codigosConvenio) {
      ...
      // Verificar existência com codigoHospital + codigoConvenio + convenioId
